...
if __name__ == '__main__':
    ...
    print("Loaded .xvg data with shape: {}".format(x.shape))
    if len(x.shape) == 1:
        x = x.reshape(-1, 1)
        print(" reshaped to: {}".format(x.shape))
    col = args.col - 1
    hist_data = x[:,col]
    data_min = hist_data.min()
    data_max = hist_data.max()

    print("hist data shape: {}".format(hist_data.shape))
    print("first, last values: {}, {}".format(hist_data[0], hist_data[-1]))
    print("min, max data values: {}, {}".format(data_min, data_max))
    print("mean, std values: {}, {}".format(hist_data.mean(), hist_data.std()))
